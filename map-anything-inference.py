...
def parse_args():
    ...
    parser.add_argument(
        "--stride", type=int, default=1, help="stride'th image to be selected "
    )
    parser.add_argument(
...
def demo_fn(args):
    ...
    if args.use_ba:
        ...
        # Remove padded regions and combine with model output masks
        # Work on CPU numpy to avoid tensor/int casting issues
        original_coords_np = (
            original_coords.detach().cpu().numpy()
            if isinstance(original_coords, torch.Tensor)
            else np.asarray(original_coords)
        )
        for i, (x1, y1, x2, y2, _, _) in enumerate(original_coords_np):
            y1 = int((y1 / img_load_resolution) * conf_mask.shape[1])
            y2 = int((y2 / img_load_resolution) * conf_mask.shape[1])
            x1 = int((x1 / img_load_resolution) * conf_mask.shape[2])
            x2 = int((x2 / img_load_resolution) * conf_mask.shape[2])
            conf_mask[i,:y1] = False
            conf_mask[i,y2:] = False
            conf_mask[i,:,:x1] = False
            conf_mask[i,:,x2:] = False
            conf_mask[i] &= masks[i]

        # At most write 100000 3D points to COLMAP reconstruction object
